      ...
      <div class="right" v-if="selectedID !== 0 && room">
        <div class="header d-flex justify-content-between">
          <b-button
            ...
            @click="
              [(profileOpened = !profileOpened), (settingsOpened = false)]
            "
          >
            <b-avatar
              v-if="room.avatar !== ''"
              :src="getImgUrl(room.avatar)"
            ></b-avatar>
          ...
          <div>
            <b-button
              v-if="!settingsOpened && !profileOpened"
            ...
            <b-button
              v-if="profileOpened && !settingsOpened"
              @click="profileOpened = false"
              variant="white"
            >
              <b-icon icon="arrow-left"></b-icon>
            </b-button>
        ...
        <div v-if="!settingsOpened && !profileOpened" class="chat-field">
          <ul
            ...
            class="chat flex-column"
          >
            <template v-for="(iter, index) in this.room.messages">
              <li :key="index" class="mb-3">
                <p></p>
                <div
                    v-if="!iter.is_deleted"
                  :class="[
                    currentUser.id == iter.user_id ? 'personal' : null,
                    currentUser.id != iter.user_id ? 'other' : null,
                    iter.is_notification ? 'notification' : null
                  ]"
                >
                  <div class="message-field d-flex">
                    <div>
                      ...
                      <div class="message-date">
                        {{
                          new Date(iter.sent).getHours() +
                          ":" +
                          new Date(iter.sent).getUTCMinutes() +
                          " " +
                          new Date(iter.sent).toDateString()
                        }}
                      </div>
                    </div>
                ...
                <div v-else :class="[
                  ...
                  <div class="message-field">
                    <p class="font-italic">Message was deleted</p>
                    <div class="message-date">
                      {{
                        new Date(iter.sent).getHours() +
                        ":" +
          ...
          <div class="bottom">
            ...
            <div class="pr-5">
              ...
              <b-icon
                v-if="message === ''"
                icon="forward"
                v-b-popover.hover.top="'Can not send message when it is empty'"
                font-scale="1.5"
              ></b-icon>
              <b-icon
                class="hover"
        ...
        <div v-if="settingsOpened" class="settings p-4">
          ...
          <div class="borders mt-5" v-if="currentProfile.is_admin">
        ...
        <div class="profile p-4" v-if="profileOpened">
          ...
          <div class="members mb-5 mt-5">
            <template v-for="(iter,index) in room.participants">
              <div  class="mem-row mb-3 d-flex justify-content-between" :key="index">
                <a class="profile-a" :href="`/user/${iter.user_id}/profile`">{{iter.user.name}} {{iter.user.last_name}}</a>
                <b-button v-if="currentProfile.is_admin" @click="kickUser(iter.user_id)" variant="danger">Kick</b-button>
              </div>
            </template>
          </div>
          <b-button class="w-100" variant="danger" @click="LeaveRoom"
            >Leave</b-button
          >
        </div>
      </div>
    ...
    <div v-if="CreatingNewGroup">
      <b-container class="mt-4">
        ...
        <div class="mt-5 d-flex justify-content-between">
          <div></div>
          <b-button variant="outline-success" @click="createNewGroup()"
            >Save</b-button
          >
        </div>
      </b-container>
...
export default {
  ...
  methods: {
    ...
    saveName() {
      window.axios
        .post("api/chatting/settings/add/name", this.room)
        .then(() => {
    ...
    saveAbout() {
      window.axios
        .post("api/chatting/settings/add/about", this.room)
        .then(() => {
    ...
    kickUser(id) {
      window.axios
          .post("api/chatting/settings/kick/user", {"rooms_id": this.selectedID, "user_id":id})
          .then(() => {
            this.makeToast("user kicked successfully", "success");
            this.getRooms();
            this.getRoom(this.selectedID);
            this.connection.send("room " + this.selectedID);
          })
    ...
    deleteMessage(id) {
      window.axios
          .post("api/chatting/message/delete", {"rooms_id": this.selectedID, "id":id})
          .then(() => {
            this.makeToast("message deleted successfully", "success");
            this.getRooms();
            this.getRoom(this.selectedID);
            this.connection.send("room " + this.selectedID);
          })
    ...
    saveAvatar() {
      ...
      window.axios
        .post(`api/chatting/settings/add/avatar`, formData, {
          headers: {
    ...
    getNonMembers() {
      window.axios
        .post("api/chatting/settings/get/non/members", { id: this.selectedID })
        .then(res => {
    ...
    findCurrentUser() {
      for (let i = 0; i < this.room.participants.length; i++) {
        if(this.room.participants[i].user_id == this.currentUser.id) {
          this.currentProfile = this.room.participants[i]
          return
        }
      }
    },
    inviteUsers() {
      window.axios
        .post("api/chatting/settings/add/members", {
          id: this.selectedID,
          participants: this.invitees
        })
        .then(() => {
    ...
    LeaveRoom() {
      window.axios
        .post("api/chatting/settings/leave/room", { id: this.selectedID })
        .then(() => {
          this.selectedID = 0;
          this.connection.send("room " + this.selectedID);
          this.getRooms();
    ...
    createNewGroup() {
      ...
      window.axios
        .post("api/chatting/new/group", this.newGroup)
        .then(() => {
    ...
    async getUnreadMessages() {
      await window.axios
        .post("api/chatting/get/rooms/unseen/chats", {
          rooms_id: this.selectedID
        })
        .then(res => {
    ...
    getRoom(id) {
      this.selectedID = id;
      window.axios
        .post("api/chatting/get/room", { id: this.selectedID })
        .then(res => {
