    ...
    <div class="init-screen">
        ...
        <div class="container">
            ...
            <div class="table-head grid">
                <div class=""><input @click="selectAllUsers"
                                     v-model="allUsersSelected" type="checkbox"></div>
            ...
            <div class="table">
                <div class="row grid" v-for="(iter, index) in users" :key="index">
                    ...
                    <div><p>{{ index}}</p></div>
                    <div><p>{{ iter.name + " " + iter.lastname}}</p></div>
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        methods: {
            joinLocation: function () {
                ...
                const vue = this;
                window.axios
                    .post("/api/location/join", {location: this.$route.params.id, user: this.currentUser.id})
                    .then(res => {
            ...
            selectAllUsers: function() {
                this.users.forEach(
                    iter => (iter.isChecked = !this.allUsersSelected)
                );
            },
            loadUsers: function () {
                this.errors = {};
            ...
            leave: function () {
                ...
                const vue = this;
                window.axios
                    .post("/api/location/leave", {location: this.$route.params.id, user: this.currentUser.id})
                    .then(() => {
