<template>
  <b-container class="mt-3">
    <div><h4>Users: {{users.length}}</h4></div>
    <hr>
      ...
      <div class="row-bottom" :key="index">
        <div class="d-flex align-content-center justify-content-between">
         <div class="d-flex align-content-center ">
          ...
          <b-button @click="$router.push(`/user/${iter.id}/profile`)" variant="outline-primary">Visit</b-button>
        </div>
        <div class="border"><p class="ml-3 mt-3">{{iter.about}}</p></div>
        <hr>
      </div>
    </template>
  </b-container>
